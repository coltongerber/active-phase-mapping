...
parser.add_argument(
    "-num_y", type=int, help="Number of y-points sampled per composition"
)
parser.add_argument(
    "-num_samples", type=int, help="Number of samples per entropy calculation"
)
parser.add_argument(
    "-num_curves",
    type=int,
    help="Number of combinations of samples per entropy calculation",
)
...
parser.add_argument(
    "-num_comps", type=int, help="number of compositions considered for next design"
)
parser.add_argument("-directory", type=str, help="path to results directory")
parser.add_argument(
    "-truth_df", type=str, help="path to DataFrame JSON containing true y-data"
)
...
parser.add_argument("-resume", type=bool, help="Resume from previous run")
...
for seed in range(args.seed_range[0], args.seed_range[1]):
    ...
    resume = args.resume
    if resume:
        master_hull_dict = pd.read_pickle(f"{args.directory}/hull.pkl")
        ...
        master_problem_setup_dict = pd.read_pickle(
            f"{args.directory}/problem_setup.pkl"
        )
        ...
        energy_error_stds = df["energy_error_stds"].to_list()
        initial_entropies = df["Entropy"].to_list()
        true_positive_rates = df["true_positive_rate"].to_list()
        false_positive_rates = df["false_positive_rate"].to_list()
        iteration_times = df["time (s)"].to_list()
        ...
        if last_it != df_last_it:
            raise ValueError(
                f"Last iteration in dataset ({last_it}) does not match last iteration in performance file ({df_last_it})"
            )
        ...
        if truth_df:
            ...
            prism_df["reduced_point"] = prism_df.apply(
                lambda x: [
                    x["S"] / 2,
                    x["Se"] / 2,
                    x["Te"] / 2,
        ...
        for i in range(num_polymorphs):
            ...
            if truth_df:
                ...
                poly_dict[i]["true_y"] = (
                    generate_true_function(design_space, knot_N) + alpha[i] * i
                )
        ...
        if (np.delete(Y_zeroed, endpoint_indices) < 0).sum() == 0:
            ...
            true_e_hull = lin_comb + get_hull_energies(
                design_space,
                Y_zeroed,
    ...
    for it in tqdm(iter_range):
        ...
        (
            ...
            endpoint_indices=endpoint_indices,
            dimensions=dimensions,
            true_hull_classifications=true_classifications,
            poly_dict=poly_dict,
            multi=True,
        ...
        for i in range(num_polymorphs):
            ...
            dict_list = do_parallel(
                ...
                poly_dict=poly_dict,
                polymorph_index=i,
                num_polymorphs=num_polymorphs,
                dimensions=dimensions,
                true_classifications=true_classifications,
                entropy_type=entropy_type,
            )
            for x in dict_list:
        ...
        if poly_dict[max_polymorph]["std"] == 0:
            y_array_unnormalized = (
                poly_dict[max_polymorph]["dataset"].y + poly_dict[max_polymorph]["mean"]
            )
        else:
            y_array_unnormalized = (
                poly_dict[max_polymorph]["dataset"].y * poly_dict[max_polymorph]["std"]
                + poly_dict[max_polymorph]["mean"]
            )
        ...
        if poly_dict[max_polymorph]["std"] == 0:
            ...
            normalized_y = (
                y_array_unnormalized - poly_dict[max_polymorph]["mean"]
            ) / poly_dict[max_polymorph]["std"]
        x_data = jnp.vstack(
            [poly_dict[max_polymorph]["dataset"].X, next_x]
        )  # [:, jnp.newaxis]
        ...
        (
            ...
            poly_dict[max_polymorph]["posterior"],
