...
config.update(
    "jax_enable_x64", True
)  # TODO: Does this need to be here? Or can it be below the other imports?
...
def get_endpoint_indices(dimensions, pts):
    ...
    for x in endpoints:
        index = np.argmax(
            (np.array(pts) == x).sum(1)
        )  # determines index corresponding to endpoint.
...
def get_hull_energies(design_space, Y, endpoint_indices, dimensions):
    ...
    for point in design_space:
        ...
        # 2a
        for eq in new_eqns:
            prospective_hull_energy = np.dot(
                new_point, eq
            )  # Get energy E=<x,1> dot <-A/B,-C/B>
...
def get_lin_comb(pts, endpoint_indices, Y):
    ...
    lin_comb = np.dot(
        pts, endpoint_energies
    )  # Gives the linear combination of all three vertices.
    return lin_comb
...
def calc_expected_energy_or_entropy(
    poly_dict=None,
    design_space=None,
    num_curves=None,
    num_samples=None,
    ...
    multi=False,
    dimensions=None,
    true_hull_classifications=None,
    entropy_type=None,
):
    if multi:
    ...
    for x in range(num_samples):
        ...
        avg_pred += E_hull
        false_positive_rate, true_positive_rate = roc(
            pred_vertices=classifications, true_vertices=true_hull_classifications
        )
...
def get_total_entropy(hull_samples, entropy_type):
    ...
    if entropy_type == "meanfield":
        ...
        total_entropy = (
            dimensions / 2 * np.log(2 * np.pi * np.e)
            + 0.5 * (slogdet(np.cov(hull_samples))[1])
        )
...
def EIG_chaase_meanfield(
    ...
    if num_y > 1:
        samples, _ = sample_from_posterior(
            pred_mean, pred_cov, design_space, num_y, envelopes=False
        )  # samples y curves.
    elif num_y == 1:
        ...
        samples.shape = (
            len(samples),
            1,
        )  # formatting as column vector so that it is compliant with rest of loop.
    ...
    for x in range(num_y):  # inner loop, ys.
        ...
        tmp_dataset = tmp_dataset + Dataset(
            X=jnp.atleast_2d(composition), y=jnp.atleast_2d(y_0)
        )  # updating dataset with new value
        # Update the model given new y-value.
        npr.seed(seed)
        rng_key = jrnd.PRNGKey(seed)
        pred_mean, pred_cov, posterior, params = update_model(
            tmp_dataset, design_space, rng_key, update_params=False
        )  # update the model.
        # Calculate the resulting entropy
        avg_e_pred, _, _, entropy, hull_samples, energy_samples = (
...
def EIG_chaase_meanfield_multi(
    composition,
    dataset,
    seed,
    index_dict,
    ...
    for x in range(num_y):  # inner loop, ys.
        ...
        y_0 = function[index]  # Get y-value for given curve
        # Make copy of dataset and add y-value.
        tmp_poly_dict[polymorph_index]["dataset"] = dataset + Dataset(
            X=jnp.atleast_2d(composition), y=jnp.atleast_2d(y_0)
        )  # updating dataset with new value
        # Update the model given new y-value.
        npr.seed(seed)
        rng_key = jrnd.PRNGKey(seed)
        (
            tmp_poly_dict[polymorph_index]["pred_mean"],
            tmp_poly_dict[polymorph_index]["pred_cov"],
            tmp_poly_dict[polymorph_index]["posterior"],
            tmp_poly_dict[polymorph_index]["params"],
        ) = update_model(
            tmp_poly_dict[polymorph_index]["dataset"],
            design_space,
            rng_key,
            update_params=False,
        )  # update the model.
        # Calculate the resulting entropy
        hull_expected_energy, _, _, entropy, hull_samples, energy_samples = (
            calc_expected_energy_or_entropy(
                poly_dict=tmp_poly_dict,
                design_space=design_space,
